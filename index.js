...
(async function process (bot) {
	for (const item of bot) {
		...
		function (err, sessionID, cookies, steamguard, oAuthToken) {
			if (err) { console.log(Colors.red('[%s] Unable to auth (Error: %s)'), logOnOptions.accountName, err); }
			if (!err) {
				console.log(Colors.yellow('[%s] Successfully logged on (Session ID: %s)'), logOnOptions.accountName, sessionID);
				...
				community.httpRequestPost(
					...
					function (err, res, data) {
						...
						if (!err) {
							//  if (data.success == 1) 
							console.log(Colors.green('[%s] Successfully voted up with response code %s'), logOnOptions.accountName, data.success);
						}
					},
					"steamcommunity"
				);
